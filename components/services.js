            ...
            {products.map((p, i) => (
              <SwiperSlide key={p.id}>
                <motion.div initial={{opacity:0,y:24}} whileInView={{opacity:1,y:0}} viewport={{once:true}}
                  transition={{delay:(i%3)*.09,duration:.65,ease}}>
                  <div className="pcard group flex flex-col h-full">
                    {/* Image */}
                    <div className="relative h-52 overflow-hidden flex-shrink-0">
                      <Image src={p.icon} alt={p.title} fill
                        className="object-cover opacity-75 group-hover:opacity-95 group-hover:scale-[1.06] transition-all duration-700"/>
                      <div className="absolute inset-0" style={{background:'linear-gradient(to top,#0D1117 0%,transparent 55%)'}}/>

                      {/* Category chip */}
                      <div className="absolute top-4 left-4">
                        <span className="text-white text-[.6rem] font-mono font-medium uppercase tracking-[.14em] px-3 py-1.5 bg-[#E3510F]"
                    ...
                    <div className="p-7 flex flex-col flex-grow">
                      <h3 className="text-[#F0F2F5] font-semibold text-[.96rem] mb-3 leading-snug group-hover:text-[#E3510F] transition-colors duration-300">{p.title}</h3>
                      <p className="text-[#5A6478] text-[.82rem] leading-relaxed mb-6 flex-grow">{strip(p.description||p.content)}</p>

                      <Link href={`/products/${p.slug}`}
                        className="flex items-center gap-2 text-[.65rem] font-mono uppercase tracking-[.14em] text-[#9BA5B4] hover:text-[#E3510F] transition-colors group/link pt-5 border-t border-white/[.05]">
