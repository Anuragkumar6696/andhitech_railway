      ...
      <div className="max-w-screen-xl mx-auto px-5 md:px-10 relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 mb-24 items-end">
          <motion.div initial={{ opacity:0, y:20 }} whileInView={{ opacity:1, y:0 }} viewport={{ once:true }}>
            ...
            <h2 className="display-md">Reasons to <span style={{color:'#B88746'}}>Work Here</span></h2>
          </motion.div>
          <motion.p initial={{ opacity:0, y:20 }} whileInView={{ opacity:1, y:0 }} viewport={{ once:true }} transition={{ delay:0.15 }}
            className="text-[#6B7A8E] text-lg leading-relaxed border-l-2 border-[#B88746]/40 pl-8 font-light">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {reasons.map(({ Icon, n, title, desc }, i) => (
            <motion.div key={i} initial={{ opacity:0, y:24 }} whileInView={{ opacity:1, y:0 }} viewport={{ once:true }}
              transition={{ delay:i*0.12 }} className="story-card p-10 group relative border-white/5 hover:border-[#B88746]/20 transition-all duration-500 shadow-xl">
              ...
              <div className="w-16 h-16 rounded-2xl bg-[#B88746]/10 flex items-center justify-center mb-10 group-hover:bg-[#B88746] transition-all duration-500 shadow-lg relative z-10 border border-[#B88746]/10">
                <Icon size={28} className="text-[#B88746] group-hover:text-white transition-colors" />
              </div>
              <div className="accent-line mb-8 w-12 h-0.5 bg-[#B88746]/30" />
              <h3 className="text-white font-bold text-xl mb-5 leading-tight tracking-tight group-hover:text-[#B88746] transition-colors" style={{fontFamily:'Inter, sans-serif'}}>{title}</h3>
              <p className="text-[#6B7A8E] text-base leading-relaxed font-light" dangerouslySetInnerHTML={{ __html:desc }} />
            </motion.div>
          ))}
        </div>
      </div>
