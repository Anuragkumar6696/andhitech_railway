...
export default function Header({ initialData }) {
  ...
  return (
    <header className={`fixed top-0 left-0 right-0 z-[900] transition-all duration-500
      ${scrolled ? 'bg-[#07080C]/92 backdrop-blur-2xl border-b border-white/[.05] py-3' : 'bg-transparent py-5'}`}>
      {/* Top flame stripe — visible when scrolled */}
      <div className={`absolute top-0 inset-x-0 h-[1.5px] transition-opacity duration-500 ${scrolled ? 'opacity-100' : 'opacity-0'}`}
      ...
      <div className="max-w-screen-xl mx-auto px-5 md:px-10 flex items-center justify-between gap-8">
        ...
        <Link href="/" className="relative z-10 flex-shrink-0">
          <Image src={settings?.logo || '/images/logo.png'} alt="AND Hitech Industries" width={160} height={50}
            className="h-10 w-auto brightness-0 invert transition-opacity duration-300 hover:opacity-80"
            priority />
        </Link>
          ...
          <motion.div
            ...
            <div className="max-w-screen-xl mx-auto px-5 py-8 space-y-px">
              {links.map(({ label, href }, i) => (
                <motion.div key={href} initial={{ opacity:0, x:-14 }} animate={{ opacity:1, x:0 }} transition={{ delay:i*.05 }}>
                  <Link href={href}
