...
export default function OurMissionVision() {
  ...
  return (
    <section className="py-24 md:py-32 bg-[#0D1117] relative overflow-hidden">
      ...
      <div className="absolute inset-0 bg-[radial-gradient(ellipse_600px_500px_at_50%_0%,rgba(184,135,70,0.06),transparent_70%)] pointer-events-none" />
      <div className="max-w-screen-xl mx-auto px-5 md:px-10 relative z-10">
        <div className="flex flex-col lg:flex-row justify-between items-end mb-16 gap-8">
          <div>
            ...
            <h2 className="display-md max-w-xl">Empowering Sustainable <span style={{color:'#B88746'}}>Growth</span></h2>
          </div>
        </div>
        <div className="flex flex-wrap gap-3 mb-10">
          {tabs.map(({ id, label, Icon }) => (
            <button key={id} onClick={() => setActive(id)}
              className={`flex items-center gap-2.5 px-6 py-3 font-medium text-[11px] uppercase tracking-[0.14em] transition-all duration-300 ${active===id ? 'bg-[#B88746] text-white' : 'bg-white/[0.04] text-[#5A6478] hover:bg-white/8 hover:text-[#F0F2F5] border border-white/6'}`}
            ...
            {curr && (
              <motion.div key={curr.id} initial={{ opacity:0, y:14 }} animate={{ opacity:1, y:0 }} exit={{ opacity:0, y:-14 }}
                ...
                <div className="lg:col-span-7 space-y-6">
                  <div className="w-12 h-12 rounded-xl bg-[#B88746]/15 flex items-center justify-center text-[#B88746]">
                    <curr.Icon size={22} />
                  </div>
                    ...
                    <ul className="space-y-4">
                      {curr.points.map((p,i) => (
                        <motion.li key={i} initial={{ opacity:0, x:-10 }} animate={{ opacity:1, x:0 }} transition={{ delay:i*0.08 }}
                          ...
                          <CheckCircle2 size={15} className="text-[#B88746] flex-shrink-0" />
