    ...
    <section className="py-20 md:py-28 bg-[#0D1117] relative">
      ...
      <div className="max-w-screen-xl mx-auto px-5 md:px-10 relative z-10">
        <div className="text-center mb-14">
          ...
          <h2 className="display-md max-w-xl mx-auto">Downloadable <span style={{ color: '#B88746' }}>Brochures</span></h2>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {brochures.map((b, i) => (
            <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
              ...
              <div className="w-14 h-14 rounded-xl bg-[#B88746]/10 flex items-center justify-center mx-auto mb-6 group-hover:bg-[#B88746] transition-colors">
                <FileText size={24} className="text-[#B88746] group-hover:text-black transition-colors" />
              </div>
              <h5 className="text-[#F0F2F5] font-semibold mb-6 group-hover:text-[#black] transition-colors">{b.title}</h5>
              {b.pdf_file
                ? <Link
                  href={b.pdf_file}
                  target="_blank"
                  download
                  className="btn-flame inline-flex items-center gap-2 text-xs py-3 px-6 !text-white group-hover:!text-white"
                >
                  <Download size={13} />
