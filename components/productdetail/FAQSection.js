  ...
  return (
    <div className="space-y-4">
      ...
      <h2 className="text-[#F0F2F5] text-2xl font-bold mb-8" style={{fontFamily:'var(--font-display)'}}>
        Frequently Asked <span style={{color:'#B88746'}}>Questions</span>
      </h2>
        ...
        {faqs.map((faq, i) => (
          <div key={i} className="rounded-xl border border-white/6 overflow-hidden">
            <button onClick={() => setOpen(open===i?null:i)}
              ...
              <span className={`font-semibold text-[15px] leading-snug transition-colors ${open===i?'text-[#B88746]':'text-[#F0F2F5] group-hover:text-[#B88746]'}`}>{faq.question}</span>
              <span className={`flex-shrink-0 ml-4 w-8 h-8 rounded-lg flex items-center justify-center transition-all ${open===i?'bg-[#B88746] text-white':'bg-white/5 text-[#5A6478]'}`}>
